...
old
*/
#define VCHIQ_CHANNEL_SIZE        (VCHIQ_SLOT_SIZE - sizeof(VCHIQ_HEADER_T))
...
old version: (just want to use old .so apart from gl)
*/
...
pull in old definition to leave old libvchiq_arm.so in place
*/
...
extern VCHIQ_STATUS_T vchiq_queue_bulk_transmit_handle(VCHIQ_SERVICE_HANDLE_T service, VCHI_MEM_HANDLE_T handle, void *offset, int size, void *userdata);
